...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Protocol Error: Registration failed");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="rounded-3xl border border-white/5 bg-white/5 p-8 backdrop-blur-2xl shadow-2xl">
          ...
          <form className="space-y-4" onSubmit={handleRegister}>
            ...
            <button 
              disabled={isLoading}
              type="submit"
              className="group relative flex w-full items-center justify-center gap-3 rounded-xl bg-gradient-to-r from-cyan-500 to-violet-600 py-4 font-black uppercase tracking-widest text-white transition-all hover:opacity-90 active:scale-95 shadow-xl shadow-cyan-500/20 disabled:opacity-50"
            >
